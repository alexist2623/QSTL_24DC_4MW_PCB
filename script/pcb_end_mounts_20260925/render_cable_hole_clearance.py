...
validation=json.loads((P/'docs/ZIF_M1p6_mount_validation.json').read_text())
...
ax.annotate('2 x diameter 1.80 mm',xy=(mounts[0]['x']-.6,mounts[0]['y']+.65),xytext=(.25,3.7),arrowprops={'arrowstyle':'->','color':'#b52225'},color='#b52225')
...
dim(ax,mounts[0]['x'],mounts[1]['x'],-.6,'16.30 mm hole center pitch')
...
detail.plot([left,left_hole_inner],[m['y']]*2,color='#b52225',lw=5)
...
detail.text((left+left_hole_inner)/2,2.5,'0.55 mm overlap',ha='center',color='#b52225',fontsize=11)
...
fig.text(.44,.235,'Clear width between inner hole edges: 14.50 mm',fontsize=12)
fig.text(.44,.197,'Cable width: 15.60 mm',fontsize=12)
fig.text(.44,.158,'Signed clearance: -0.55 mm on each side (overlap)',fontsize=12,color='#b52225',fontweight='bold')
...
fig.text(.06,.025,'PCB '+validation['pcb_sha256'][:24]+'  |  Hole positions unchanged; no fit clearance has been added.',fontsize=9,color='#56636f')
