...
try {
 ...
 foreach($spec in (,@('Socket Head','ISO 4762',4,'ISO_4762_M1p6x4.ipt'))){
  if($spec[0] -eq 'Plain'){
 ...
 $report|ConvertTo-Json -Depth 8|Set-Content -LiteralPath (Join-Path $model 'm1p6_member.json') -Encoding UTF8
