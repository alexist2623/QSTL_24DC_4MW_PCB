"""Read-only acceptance checks for the two M1.6 PCB end mounts."""
...
for p in mounts:
    ...
    expected=[1.6 if p['number']=='MH7' else 17.9,1.2]+[2.2]*6+[1.8]
...
for layer in (1,2,3,4,5,32):
    ...
    for p in mounts:
        ...
        ring=Point(x,y).buffer(1.15,quad_segs=256).difference(Point(x,y).buffer(1.08,quad_segs=256))
...
report=dict(passed=not errors,errors=errors,pcb_sha256=sha(source),source_sha256=sha(H/'before_mounts.PcbDoc'),
    unchanged_streams=preserved,mounts=mount_rows,hole_pitch_mm=16.3,board_outline_mm=[19.5,67.9],
    nearest_drill_to_edge_mm=.3,mask_symmetric_difference_mm2=mask_delta,mechanical_solid_connection_checks=contacts,
...
if not errors:(P/'docs/ZIF_M1p6_mount_validation.json').write_text(json.dumps(report,indent=2))
