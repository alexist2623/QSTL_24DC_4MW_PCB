...
try{
 ...
 $member=Get-Content -LiteralPath (Join-Path $out 'm1p6_member.json') -Raw|ConvertFrom-Json
 if((Get-FileHash -LiteralPath $member.file).Hash-ne$member.sha256){throw 'Changed standard hardware.'}
 $path=Join-Path $out 'ZIF_M1p6_Fit.iam'
 ...
 $screw=$app.Documents.Open($member.file,$false);ExportStep $screw 'ISO_4762_M1p6x4'
 ...
 foreach($hole in $inputs.mounts){
  ...
  $o=$occs.Add($member.file,$m);$o.Name=$hole.name+'_ISO4762_M1p6x4';$o.Grounded=$true;$screws+=,$o
 }
 ...
 $app.Visible=$true
 Screenshot $doc 'ZIF_M1p6_Top' @(0.975,0.1,8) @(0.975,0.1,.16) @(2.6,1.9)
 Screenshot $doc 'ZIF_M1p6_Oblique' @(3,-3,5) @(0.975,0.1,.16) @(2.8,2)
 $c=@($occs|Where-Object {$_.Name -like 'FPC*'})[0];$c.Visible=$false
 Screenshot $doc 'ZIF_M1p6_NoCable' @(0.975,-3,4) @(0.975,.34,.16) @(2.4,1.725)
 $c.Visible=$true;$doc.Save();ExportStep $doc 'ZIF_M1p6_Fit'
