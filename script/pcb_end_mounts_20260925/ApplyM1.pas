...
Begin
 ...
 Try
  ...
  While P<>Nil Do Begin
   If (P.Name='MH7') Or (P.Name='MH8') Then Begin
    If P.Name='MH7' Then P.X:=MMsToCoord(1.05) Else P.X:=MMsToCoord(18.45);P.Y:=MMsToCoord(1.2);P.HoleSize:=MMsToCoord(1.2);
    P.TopXSize:=MMsToCoord(1.6);P.TopYSize:=MMsToCoord(1.6);
    P.MidXSize:=MMsToCoord(1.6);P.MidYSize:=MMsToCoord(1.6);
    P.BotXSize:=MMsToCoord(1.6);P.BotYSize:=MMsToCoord(1.6);Inc(Count);
   End;
   P:=I.NextPCBObject;
 ...
 L:=TStringList.Create;L.Add('M1;HOLE=1.2;LAND=1.6;X=1.05/18.45;Y=1.2;COUNT='+IntToStr(Count));L.Add('COMPLETE');
 L.SaveToFile('C:\JeonghyunPark\Workspace\QSTL_24DC_4MW_PCB\script\pcb_end_mounts_20260925\resize_m1_native.txt');L.Free;
End;
