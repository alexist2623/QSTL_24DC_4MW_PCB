...
try {
 ...
 foreach($spec in (,@('Round Head','KS B 1021 - Metric',4,'KS_B1021_M1x4.ipt'))){
  if($spec[0] -eq 'Plain'){
  ...
  $matches=@();for($i=1;$i -le $family.TableRows.Count;$i++){
   ...
   if([double]$row.GetCellValue('NND') -eq 1 -and ($spec[0] -eq 'Plain' -or [double]$row.GetCellValue('NLG') -eq $spec[2])){$matches+= $i}
  }
 ...
 $report|ConvertTo-Json -Depth 8|Set-Content -LiteralPath (Join-Path $model 'm1_round_member.json') -Encoding UTF8
