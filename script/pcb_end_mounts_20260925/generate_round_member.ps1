...
$model=Join-Path $root 'QSTL_24DC_4MW_PCB\Mechanical_Assembly\ZIF_M1p6_Review'
...
try {
 ...
 foreach($spec in (,@('Round Head','ISO 7045 H',4,'ISO_7045_H_M1p6x4.ipt'))){
  if($spec[0] -eq 'Plain'){
  ...
  $matches=@();for($i=1;$i -le $family.TableRows.Count;$i++){
   ...
   if([double]$row.GetCellValue('NND') -eq 1.6 -and ($spec[0] -eq 'Plain' -or [double]$row.GetCellValue('NLG') -eq $spec[2])){$matches+= $i}
  }
 ...
 $report|ConvertTo-Json -Depth 8|Set-Content -LiteralPath (Join-Path $model 'm1p6_round_member.json') -Encoding UTF8
