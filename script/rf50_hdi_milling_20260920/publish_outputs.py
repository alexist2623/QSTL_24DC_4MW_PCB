...
def render(bounds,out,title,subtitle,layers=(32,),scale=64,labels=True):
    ...
    im.save(out)
render((0,25,19.5,52),D/'RF_only.png','RF6 | 50-ohm geometry and blind-via fences','C1-C6: no through vias | 78 GND shield vias | 2 mm nominal fence pitch')
...
F=H/'fabrication';shield=[q for q in n['vias'] if q['net']=='GND'];assert len(shield)==78
...
manifest=dict(status='Validated design companion; complete production CAM export not included',pcb_sha256=n['source_sha256'],pcblib_sha256=v['pcblib_sha256'],schematic_sha256=v['schematic_sha256'],files={f.name:hashlib.sha256(f.read_bytes()).hexdigest() for f in dest.iterdir() if f.is_file() and f.name!='manifest.json'},validation='../../docs/RF50_HDI_validation.json')
(dest/'manifest.json').write_text(json.dumps(manifest,indent=2)+'\n')
shutil.copy2(H/'native_paste_audit.txt',D/'native_paste_audit.txt')
shutil.copy2(P/'work/rf50_reopen_check.txt',D/'native_reopen_check.txt')
