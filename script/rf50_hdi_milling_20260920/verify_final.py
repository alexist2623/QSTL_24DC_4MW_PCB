...
check((len(cs),len(ps),len(ns),len(vs))==(22,147,37,138),'Counts differ')
...
check(len(shield)==78 and all(tuple(sorted((v['body'][29],v['body'][30])))==(5,32) for v in shield),'Shield via span/count')
check(len(signal)==60 and all(tuple(sorted((v['body'][29],v['body'][30])))==(1,32) for v in signal),'Signal via span/count')
for v in shield:
...
html=(D/'RF50_HDI_DRC.html').read_text(encoding='cp949',errors='replace');rows=re.findall(r'<td class="column1"><a href="#[^"]+">(.*?)</a></td>\s*<td class="column2">(\d+)</td>',html,re.S)
