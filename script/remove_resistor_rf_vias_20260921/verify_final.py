...
for p in ps:
    ...
    if c:
        ...
        if c=='Q1' or (c.startswith('R') and p['number']=='2'):check(len(matches)==1 and matches[0]['net']==net,f'Via-in-pad missing {c}.{p["number"]}')
        if c.startswith('R') and p['number']=='1':check(not matches,f'RF-side resistor via remains {c}.{p["number"]}')
        if c.startswith('C'):
...
check((len(cs),len(ps),len(ns),len(vs))==(22,147,37,524),'Counts differ')
...
check(len(shield)==470 and all(tuple(sorted((v['body'][29],v['body'][30])))==(5,32) for v in shield),'Shield via span/count')
pairgap=min(math.dist((a['x'],a['y']),(b['x'],b['y']))-(a['diameter']+b['diameter'])/2 for i,a in enumerate(shield) for b in shield[:i]);check(pairgap>=.15-6*UNIT,'Shield-to-shield spacing');check(len(signal)==54 and all(tuple(sorted((v['body'][29],v['body'][30])))==(1,32) for v in signal),'Signal via span/count')
...
html=(D/'Resistor_RF_Via_Removal_DRC.html').read_text(encoding='cp949',errors='replace');rows=re.findall(r'<td class="column1"><a href="#[^"]+">(.*?)</a></td>\s*<td class="column2">(\d+)</td>',html,re.S)
