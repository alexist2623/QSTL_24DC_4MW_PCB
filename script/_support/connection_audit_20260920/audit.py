...
H=Path(__file__).resolve().parent;W=H.parent;PREV=W/'dc_direct_20260920';P=Path(r'C:\JeonghyunPark\Workspace\QSTL_24DC_4MW_PCB\QSTL_24DC_4MW_PCB');B='QSTL_24DC_4MW_PCB'
...
if not (H/(B+'.PcbDoc')).exists():
 assert sha(P/(B+'.PcbDoc'))=='1cc6aafc06314555b8206005bea64ebd285a8434f9bc522d36a8c283f3d84aa5'
 for ext in ['.PcbDoc','.SchDoc','.SchLib','.PrjPcb','.PcbLib']:shutil.copy2(P/(B+ext),H/(B+ext))
source=(P if '--after' in sys.argv else H)/(B+'.PcbDoc');s=snapshot(source);cs=properties(s['Components6/Data']);ns=properties(s['Nets6/Data']);ps=pads_stream(s['Pads6/Data']);ts,ars,vs=read_route(s,ns);regions=read_regions(s['Regions6/Data']);polygons=properties(s['Polygons6/Data']);expected=json.loads((PREV/'expected.json').read_text())
...
for v in vs:
 assert (v['body'][29],v['body'][30])==(1,32),'Unexpected via layer span'
 insert(v['net'],'via:'+str(v['index']),layers,Point(v['x'],v['y']).buffer(v['diameter']/2,quad_segs=128))
...
report=dict(passed=not broken and not connections,physical_copper_connectivity_passed=not broken,pcb_sha256=sha(source),method='Independent graph of saved copper intersections by net and physical layer, including through-via spans and GND polygon regions with inherited polygon net assignments; no ratsnest/DRC result used to infer continuity.',connected_net_count=len(reports),DC_net_count=sum(r['net'].startswith('ZIF') for r in reports),RF_named_net_count=sum(r['net'].startswith(('MW','S')) for r in reports),all_component_pin_assignments_match_schematic=True,disconnected_copper_nets=broken,via_spans_top_to_bottom=len(vs),nets=reports,stored_connection_line_count=len(connections),stored_connection_lines=connections)
name='after.json' if '--after' in sys.argv else 'before.json';(H/name).write_text(json.dumps(report,indent=2));print(json.dumps({k:v for k,v in report.items() if k!='nets'},indent=2))
if '--after' in sys.argv:
 assert report['passed'];(P/'docs/connection_validation.json').write_text(json.dumps(report,indent=2)+'\n')
