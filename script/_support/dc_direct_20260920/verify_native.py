...
check((len(cs),len(ps),len(ns),len(vs))==(22,147,33,65),'Object count mismatch')
...
planpads={p['component']+'-'+p['number']:p for p in g['pads'] if not p['component'].startswith('MOUNT')};actual={cs[p['component']]['SOURCEDESIGNATOR']+'-'+p['number']:p for p in ps if p['component']!=65535}
vip=[]
for key,p in actual.items():
 ...
 if key.startswith(('Q1-','R','C')) and key.split('-')[0] not in ['R5','R6','C5','C6']:
...
check(len(ars)==len(rf['arcs'])==11,'RF arc count mismatch')
...
compilefile=P/'work/dc_direct_schematic_compile.txt';sch=validate(P/(B+'.SchDoc'),e,compilefile if compilefile.exists() else None,P/(B+'.PcbDoc'));check(sch['passed'],'Schematic mismatch: '+str(sch['errors']))
text=(D/'Connections_DRC.html').read_text(encoding='cp949',errors='replace');rows=re.findall(r'<td class="column1"><a href="#[^"]+">(.*?)</a></td>\s*<td class="column2">(\d+)</td>',text,re.S)
check(len(rows)==13 and sum(int(n) for _,n in rows)==0,'Native DRC violations remain')
report=dict(passed=not errors,errors=errors,pcb_sha256=sha(P/(B+'.PcbDoc')),pcblib_sha256=sha(P/(B+'.PcbLib')),schematic_sha256=sha(P/(B+'.SchDoc')),components=22,pcb_pads=147,named_nets=33,via_count=len(vs),ground_shield_vias=0,QD=dict(inner_pad_end_opening_mm=[4.3,4.3],pad_size_mm=[1,.5],pad_pitch_mm=.7,moved_to_top_edge=[1,24],via_in_pad=[x for x in vip if x['pin'].startswith('Q1-')],signal_interior_crossings=intrusions,library_matched=n==24),bias_via_in_pad=[x for x in vip if not x['pin'].startswith('Q1-')],capacitor_package='0402',resistor_package='0603',paste=paste,top_and_bottom_tented=len(vs),DRC_after_save_and_reopen=dict(violations=sum(int(n) for _,n in rows),checked_rules=len(rows),report='Connections_DRC.html',rules=dict(rows)),rf_qd_map=g['rf_qd_map'],zif_to_target=dc['zif_to_target'],RF_delay_matching='Deferred by user; short simple routes prioritized',RF_main_planar_lengths_mm={str(k):v['main_length_mm'] for k,v in rf['channels'].items()},route_geometry_comparison=route_checks,mask_exception='Via-in-pad holes share Bottom pad solder-mask openings; all via-specific top/bottom tent flags and paste apertures are present.')
