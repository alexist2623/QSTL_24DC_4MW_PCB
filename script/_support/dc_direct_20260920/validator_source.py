...
def validate(doc,expected,report=None,pcb=None):
    ...
    check(len(labels)==len(expected['pin_net_map']) and len(segments)==len(expected['pin_net_map']),'Expected named-pin label stubs')
    check(len(nc_points)==len(expected['nc_pins']),'Expected intentional NC markers')
    ...
    if report and report.exists():
        ...
        check(summary.get('NET_COUNT')==str(expected['native_compiled_expected_net_count']),'Expected named nets plus singleton NC nets')
