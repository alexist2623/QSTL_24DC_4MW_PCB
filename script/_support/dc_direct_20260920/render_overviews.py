...
for p in n['pads']:
 ...
 color='#f18bd2' if p['net'] and p['net'].startswith('MW') else '#e7c786';d.polygon([xy(*v) for v in M.corners(p['x'],p['y'],p['size_x'],p['size_y'],p['rotation'])],fill=color)
 if p['hole']:
...
for c in n['components']:
 if c['designator'] in ['C2','C3','C4','R2','R3','R4','C6','R6']:
  x,y=xy(*{'R2':(16.,44.75),'R3':(13.65,48.8),'R4':(5.85,48.8),'R6':(3.5,44.75),'C2':(17.55,41.25),'C3':(15.2,45.45),'C4':(4.3,45.45),'C6':(1.95,41.25)}[c['designator']]);d.text((x,y),c['designator'],font=small,fill='white',anchor='mm')
d.text((35,1074),'24 centred QD vias + 16 centred R/C vias. 0 shield vias. Native DRC: 0 violations.',font=small,fill='#b8c9d3');im.save(D/'QD_detail.png');(H/'native_render_snapshot.json').write_text(json.dumps(n,indent=2));print('Native PCB previews saved:',n['source_sha256'])
