...
$here=Join-Path $root 'script\dwg_gerber_refresh_20260924'
...
function Frame($code,$title,$number,$scale,$material='OXYGEN-FREE COPPER'){
 ...
 TB 146 6 'REV E' 2.2;TB 168 6 ($number.ToString()+' / 5') 2.2
...
try{
 ...
 $doc.SaveAs((Join-Path $out 'QSTL_Split_Mount_RevE.idw'),$false)
 $doc.SaveAsInventorDWG((Join-Path $out 'QSTL_Split_Mount_RevE_Inventor.dwg'),$true)
 ...
 $media.FileName=Join-Path $out 'QSTL_Split_Mount_RevE.pdf';$add.SaveCopyAs($doc,$ctx,$opt,$media)
 foreach($s in $doc.Sheets){
  $code=$s.Name.Split(':')[0];$path=Join-Path $out ('QSTL_Split_Mount_RevE_'+$code+'.dwg')
