...
H=Path(__file__).resolve().parent;P=H.parents[1]/'QSTL_24DC_4MW_PCB';D=P/'docs';W=H.parent/'_support';B='QSTL_24DC_4MW_PCB';PREV=H
...
check(collections.Counter(map(viakey,signal))==collections.Counter(map(viakey,[v for v in wanted['vias'] if v['net']!='GND'])),'Unexpected via addition, deletion or movement')
for stream in ('Fills6/Data',):
...
cx,cy=json.loads((H.parent/'rf_six_inward_20260920/geometry.json').read_text())['qd_component_center']
while pos<len(raw):
...
html=(D/'DC_Even_ZIF_DRC.html').read_text(encoding='cp949',errors='replace');rows=re.findall(r'<td class="column1"><a href="#[^"]+">(.*?)</a></td>\s*<td class="column2">(\d+)</td>',html,re.S)
...
if report['passed']:(D/'validation.json').write_text(json.dumps(report,indent=2));(D/'DC_Even_ZIF_validation.json').write_text(json.dumps(report,indent=2))
