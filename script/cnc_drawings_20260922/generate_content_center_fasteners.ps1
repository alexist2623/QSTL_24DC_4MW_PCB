...
try {
 ...
 foreach($spec in @(@('Countersunk','DIN 7991',10,'DIN_7991_M3x10.ipt'),@('Socket Head','ISO 4762',8,'ISO_4762_M3x8.ipt'))){
  if($spec[0] -eq 'Plain'){
  ...
  $matches=@();for($i=1;$i -le $family.TableRows.Count;$i++){
   ...
   if([double]$row.GetCellValue('NND') -eq 3 -and ($spec[0] -eq 'Plain' -or [double]$row.GetCellValue('NLG') -eq $spec[2])){$matches+= $i}
  }
  if($matches.Count -ne 1){throw 'Expected one exact standard member.'}
 ...
 $report|ConvertTo-Json -Depth 8|Set-Content -LiteralPath (Join-Path $model 'content_center_fasteners.json') -Encoding UTF8
