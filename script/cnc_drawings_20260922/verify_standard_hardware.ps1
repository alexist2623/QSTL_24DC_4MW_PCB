...
try {
 ...
 $d=$app.Documents.Open((Join-Path $model 'Standard_Fasteners\DIN_7991_M3x10.ipt'),$true);$d.Activate()
 $cam=$app.ActiveView.Camera;$cam.Perspective=$false;$cam.Target=$tg.CreatePoint(0,0,-.5);$cam.Eye=$tg.CreatePoint(2,-3,1.5);$cam.UpVector=$tg.CreateUnitVector(0,0,1);$cam.Fit();$cam.ApplyWithoutTransition()
 $cam.SaveAsBitmap((Join-Path $model 'Standard_Fasteners\DIN_7991_M3x10.png'),900,900,$to.CreateColor(245,247,250),$to.CreateColor(245,247,250))
 Write-Output 'Verified supplied Content Center identity, original member hashes, M3x0.5-6g threads, three assembly references and zero washers.'
