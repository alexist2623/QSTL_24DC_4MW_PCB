...
function Frame($code,$title,$number,$scale,$material='OXYGEN-FREE COPPER'){
 ...
 TB 146 18 'DATE' 1.8;TB 146 14 '2026-09-22' 2.3
 TB 3 6 'THIRD ANGLE PROJECTION' 2.2
 TB 63 6 ('SCALE: '+$scale) 2.2
 TB 146 6 'REV B' 2.2;TB 168 6 ($number.ToString()+' / 5') 2.2
...
try{
 ...
 foreach($hand in @('Left','Right')){
  ...
  $null=Dim $end @($fx,0,8.5) @($fx,0,12.5) V 60 247 4
  $null=Dim $end @($wx,0,4) @($(if($isLeft){12}else{39}),0,4) H 92 259 6
  Lead $v @($cx,65,12.5) 117 219 "4X Ø3.40 THRU`nTHROUGH 4.00 FLANGE"
 ...
 $doc.SaveAs((Join-Path $out 'QSTL_Split_Mount_RevB.idw'),$false)
 $doc.SaveAsInventorDWG((Join-Path $out 'QSTL_Split_Mount_RevB_Inventor.dwg'),$true)
 ...
 $media.FileName=Join-Path $out 'QSTL_Split_Mount_RevB.pdf';$add.SaveCopyAs($doc,$ctx,$opt,$media)
 foreach($s in $doc.Sheets){
  $code=$s.Name.Split(':')[0];$path=Join-Path $out ('QSTL_Split_Mount_RevB_'+$code+'.dwg')
