...
for p in ps:
    ...
    if c:
        ...
        if c=='Q1' or c.startswith('R'):check(len(matches)==1 and matches[0]['net']==net,f'Via-in-pad missing {c}.{p["number"]}')
        if c.startswith('C'):
...
check((len(cs),len(ps),len(ns),len(vs))==(22,147,37,347),'Counts differ')
check(properties(s['Components6/Data'])==properties(old['Components6/Data']),'Component placement metadata changed')
...
check(len(shield)==287 and all(tuple(sorted((v['body'][29],v['body'][30])))==(5,32) for v in shield),'Shield via span/count')
pairgap=min(math.dist((a['x'],a['y']),(b['x'],b['y']))-(a['diameter']+b['diameter'])/2 for i,a in enumerate(shield) for b in shield[:i]);check(pairgap>=.15-6*UNIT,'Shield-to-shield spacing');check(len(signal)==60 and all(tuple(sorted((v['body'][29],v['body'][30])))==(1,32) for v in signal),'Signal via span/count')
for v in shield:
...
check(edgegap>=.33-6*UNIT,'RF fence edge gap')
...
solid=next(r for r in rules if r['NAME']=='ZIF24V2_GND_VIA_SOLID');check(solid['CONNECTSTYLE']=='Direct' and solid['SCOPE1EXPRESSION']=="IsVia And InNet('GND')",'Shield solid rule')
...
html=(D/'Shield_spacing_DRC.html').read_text(encoding='cp949',errors='replace');rows=re.findall(r'<td class="column1"><a href="#[^"]+">(.*?)</a></td>\s*<td class="column2">(\d+)</td>',html,re.S)
