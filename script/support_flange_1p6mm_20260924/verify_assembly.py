...
for r in records:
    ...
    if r['name'] == 'Existing_Carrier_Hung_From_Side_Slots' or r['name'].startswith('Device_Clamp_M3_'):
        ...
        if r['name'].startswith('Rod_Clamp_M3_'):
            before = before.copy(); before[2,3] += 2.4
...
# Dedicated end section exposes the reference point hidden by crossbars in the
# literal native top view. All coloured areas come from actual saved STEP cuts.
fig, ax = plt.subplots(figsize=(9, 10), facecolor='#f6f8fb')
fig.subplots_adjust(left=.045, right=.955, bottom=.15, top=.865)
geo = {'Tube': tube, 'Rods': frame, 'Plate / boss': centre_plate,
       'Supports': compound((left_support, right_support)),
       'Holder': transform(top, new_measure['carrier_matrix'] @ top_local),
       'PCB': transform(new_measure['board'], new_measure['pcb_matrix'])}
for name, shape in geo.items():
    for path in section_paths(shape, float(new_measure['target'][1])):
        ax.add_patch(PathPatch(path, facecolor=colors[name], edgecolor='none'))
cx, cz = new_measure['centre'][[0, 2]]
px, pz = new_measure['target'][[0, 2]]
ax.axvline(cx, ls=(0,(5,4)), lw=.8, color='#ab3a5c')
ax.axhline(cz, ls=(0,(5,4)), lw=.8, color='#ab3a5c')
ax.plot(cx, cz, 'o', ms=11, mfc='white', mec='#ab3a5c', mew=1.8, zorder=10)
ax.plot(px, pz, '+', ms=9, mew=1.8, color='#087f94', zorder=11)
ax.annotate('O = P\nRadial offset: 0.000 mm', xy=(px,pz), xytext=(26.5,17),
            fontsize=12, weight='bold', color='#087f94',
            arrowprops=dict(arrowstyle='-',color='#087f94',lw=1.3))
ax.set_xlim(-5,56);ax.set_ylim(33,-27);ax.set_aspect('equal');ax.axis('off')
fig.text(.05,.957,'Cylinder centre alignment',fontsize=20,weight='bold',color='#233348')
fig.text(.05,.915,'Actual CAD section at QD pocket | ID 51 / OD 54 mm',fontsize=11,color='#526276')
fig.text(.05,.886,'1.60 mm flanges | C1.6 x 45 deg | Plain diameter 3.4 mm holes',fontsize=10.5,color='#526276')
fig.text(.05,.106,'O: cylinder axis    P: pocket floor + 0.300 mm toward opening',fontsize=10.5,color='#233348')
fig.text(.05,.073,'Nominal CAD alignment; not a measured manufacturing tolerance.',fontsize=10,color='#526276')
fig.text(.05,.038,'Blue: deck/boss   Teal: supports   Brown: rods   Orange: holder   Green: PCB',fontsize=9.5,color='#526276')
fig.savefig(OUT/'Cylinder_centre_1p6mm.png',dpi=160,facecolor=fig.get_facecolor())
plt.close(fig)
print('Saved Cylinder_centre_1p6mm.png.',flush=True)
