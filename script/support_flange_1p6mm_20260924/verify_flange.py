...
for side in ('Left','Right'):
    old=solid(read(H.parent/'support_flange_2mm_20260924/before'/(side+'_Rod_Support.step')));new=solid(read(M/(side+'_Rod_Support.step')))
    assert inspect(new)['valid']
    thin=BRepPrimAPI_MakeBox(gp_Pnt(0 if side=='Left' else 45,0,8.5),6,80,2.4).Shape()
    xx=(0,1.6,0) if side=='Left' else (51,49.4,51)
    poly=BRepBuilderAPI_MakePolygon()
    for x,z in zip(xx,(10.9,10.9,12.5)):poly.Add(gp_Pnt(x,0,z))
    ...
    assert len(bevels)==1 and abs(bevels[0]['bounds'][5]-bevels[0]['bounds'][2]-1.6)<1e-6
    reports.append(dict(part=side,flange_thickness_mm=1.6,chamfer_leg_mm=1.6,chamfer_angle_deg=45,volume_difference_from_requested_cut_mm3=delta,matching_point_classifications=count,rod_side_face_area_before_mm2=oldarea,rod_side_face_area_after_mm2=newarea,removed_volume_mm3=volume(old)-volume(new),bevel_bounds_mm=bevels[0]['bounds']))
...
for r in placements:
    ...
    sf=list(plane_faces(screw,-1.6));pf=list(plane_faces(solid,-1.6));available=sum(area(f) for f in sf);contact=sum(area(BRepAlgoAPI_Common(a,b).Shape()) for a in sf for b in pf)
    assert available>1 and contact>0
    bearings.append(dict(screw=r['name'],underhead_plane_z_mm=-1.6,bearing_face_area_mm2=available,support_contact_area_mm2=contact,contact_fraction=contact/available))
...
report=dict(passed=True,parts=reports,central_plate_and_standard_hardware_unchanged=True,rod_screw_bearing=bearings,comment='C1.6 bevel preserves the full rod contact plane but removes part of each rod-screw head bearing footprint. This report describes geometry, not preload or structural qualification.')
(H/'flange_validation.json').write_text(json.dumps(report,indent=2));(M/'support_flange_1p6mm_validation.json').write_text(json.dumps(report,indent=2));print(json.dumps(report,indent=2))
