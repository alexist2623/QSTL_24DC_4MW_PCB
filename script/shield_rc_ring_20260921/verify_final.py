...
check((len(cs),len(ps),len(ns),len(vs))==(22,147,37,473),'Counts differ')
...
check(len(shield)==413 and all(tuple(sorted((v['body'][29],v['body'][30])))==(5,32) for v in shield),'Shield via span/count')
...
html=(D/'Shield_RC_Ring_DRC.html').read_text(encoding='cp949',errors='replace');rows=re.findall(r'<td class="column1"><a href="#[^"]+">(.*?)</a></td>\s*<td class="column2">(\d+)</td>',html,re.S)
...
if report['passed']:(D/'validation.json').write_text(json.dumps(report,indent=2));(D/'Shield_RC_Ring_validation.json').write_text(json.dumps(report,indent=2))
