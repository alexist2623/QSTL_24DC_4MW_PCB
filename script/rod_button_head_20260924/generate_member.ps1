...
try {
 ...
 foreach($spec in (,@('Round Head','ISO 7380-1',8,'ISO_7380_1_M3x8.ipt'))){
  if($spec[0] -eq 'Plain'){
 ...
 $report|ConvertTo-Json -Depth 8|Set-Content -LiteralPath (Join-Path $model 'button_head_member.json') -Encoding UTF8
