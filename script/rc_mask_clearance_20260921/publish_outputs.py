...
manifest=dict(status='Validated design companion; complete production CAM export not included',pcb_sha256=n['source_sha256'],pcblib_sha256=v['pcblib_sha256'],schematic_sha256=v['schematic_sha256'],files={f.name:hashlib.sha256(f.read_bytes()).hexdigest() for f in dest.iterdir() if f.is_file() and f.name!='manifest.json'},validation='../../docs/RC_Mask_Clearance_validation.json')
(dest/'manifest.json').write_text(json.dumps(manifest,indent=2)+'\n')
shutil.copy2(H/'native_paste_audit.txt',D/'native_paste_audit.txt')
shutil.copy2(P/'work/rc_mask_clearance_reopen_check.txt',D/'native_reopen_check.txt')
