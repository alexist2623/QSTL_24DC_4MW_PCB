...
check((len(cs),len(ps),len(ns),len(vs))==(22,147,37,530),'Counts differ')
...
html=(D/'RC_Mask_Clearance_DRC.html').read_text(encoding='cp949',errors='replace');rows=re.findall(r'<td class="column1"><a href="#[^"]+">(.*?)</a></td>\s*<td class="column2">(\d+)</td>',html,re.S)
...
if report['passed']:(D/'validation.json').write_text(json.dumps(report,indent=2));(D/'RC_Mask_Clearance_validation.json').write_text(json.dumps(report,indent=2))
