...
function Frame($code,$title,$number,$scale,$material='OXYGEN-FREE COPPER'){
 ...
 TB 146 6 'REV C' 2.2;TB 168 6 ($number.ToString()+' / 5') 2.2
...
try{
 ...
 foreach($hand in @('Left','Right')){
  ...
  $bx=if($isLeft){2.0}else{49.0}
  $null=Dim $end @($bx,0,10.5) @($fx,0,12.5) V 55 249 2
  Lead $end @($bx,0,10.5) 140 274 'C2 X 45 DEG - OUTER EDGE'
  $null=Dim $end @($wx,0,4) @($(if($isLeft){12}else{39}),0,4) H 92 259 6
  Lead $v @($cx,65,12.5) 117 219 "4X Ø3.40 THRU`nTHROUGH 2.00 FLANGE"
 ...
 $doc.SaveAs((Join-Path $out 'QSTL_Split_Mount_RevC.idw'),$false)
 $doc.SaveAsInventorDWG((Join-Path $out 'QSTL_Split_Mount_RevC_Inventor.dwg'),$true)
 ...
 $media.FileName=Join-Path $out 'QSTL_Split_Mount_RevC.pdf';$add.SaveCopyAs($doc,$ctx,$opt,$media)
 foreach($s in $doc.Sheets){
  $code=$s.Name.Split(':')[0];$path=Join-Path $out ('QSTL_Split_Mount_RevC_'+$code+'.dwg')
